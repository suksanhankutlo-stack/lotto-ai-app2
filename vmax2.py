...
class EnsembleEngine:
    ...
    def evaluate_past_10(self, progress_bar=None, status_text=None):
        ...
        for step, i in enumerate(range(start_idx, n_total)):
            ...
            hot_top = [int(idx) for idx in np.argsort(top_score)[::-1][:7]]
            hot_bot = [int(idx) for idx in np.argsort(bot_score)[::-1][:7]]
            ...
            top_hit = any(int(d) in hot_top for d in actual_3d)
            bot_hit = any(int(d) in hot_bot for d in actual_2d)
...
def combine_top_n(preds, positions, n=7):
...
st.markdown('<div class="sub-title">HYBRID EQUATION + STATISTICAL ENSEMBLE<br>AI + Statistics + Equation Discovery + Strict Walk-Forward</div>', unsafe_allow_html=True)
st.divider()
...
if st.button("🚀 วิเคราะห์เลขเด่นด้วย AI + สมการ", type="primary", use_container_width=True):
    ...
    # ====================================================
    st.divider()
    st.info(f"📅 วิเคราะห์งวดเป้าหมาย: วัน{days[next_date.dayofweek]} {next_date.strftime('%d-%m-%Y')} (ข้อมูล {len(df)} งวด)")
    ...
    for pos in ["H", "T", "O", "T2", "O2"]:
        ...
        st.markdown(
            ...
            <div class="hot-card">
                ...
                <div style="font-size:13px; color:#888; text-align:center;">{nums_prob(res["Final"])}</div>
            </div>
            <div class="info-row">🤖 <b>AI TOP-3:</b> &nbsp; {html_badge(res["AI"], "badge-ai")}</div>
            <div class="info-row">📊 <b>Frequency TOP-3:</b> &nbsp; {html_badge(res["Freq"], "badge-stat")}</div>
            <div class="info-row">🔄 <b>Transition TOP-3:</b> &nbsp; {html_badge(res["Transition"], "badge-stat")}</div>
            <div class="info-row">🧩 <b>Pattern TOP-3:</b> &nbsp; {html_badge(res["Pattern"], "badge-stat")}</div>
            <div class="info-row">🧮 <b>Equation TOP-5:</b> &nbsp; {html_badge(res["Equation"], "badge-eq")}</div>
            <div style="font-size:13px; color:#777; margin-top:8px;">
                🧮 สมการผ่าน Stability: <b>{res["StableEquations"]}</b> / {res["TotalEquations"]} &nbsp; | &nbsp; Strength: <b>{res["EquationStrength"]:.0%}</b>
            </div>
    ...
    # ====================================================
    # OVERALL (เปลี่ยนเป็น 7-TOP)
    # ====================================================
    hot_top = combine_top_n(preds, ["H", "T", "O"], 7)
    hot_bottom = combine_top_n(preds, ["T2", "O2"], 7)
    ...
    st.markdown(
        f"""
        <div class="hot-card">
            <div style="font-weight:700; color:#d32f2f;">🔥 HOT 7-TOP รูด/วิ่งบน</div>
    ...
    st.markdown(
        f"""
        <div class="hot-card">
            <div style="font-weight:700; color:#d32f2f;">🔥 HOT 7-TOP รูด/วิ่งล่าง</div>
    ...
    if past_records:
        st.write("")
        st.subheader("📜 ประวัติย้อนหลัง 10 งวด (Backtest 7-TOP)")
        
        for rec in past_records:
            top_mark = "✅ <span style='color:green;font-weight:bold;'>เข้า</span>" if rec["Top_Hit"] else "❌ <span style='color:red;'>หลุด</span>"
            bot_mark = "✅ <span style='color:green;font-weight:bold;'>เข้า</span>" if rec["Bot_Hit"] else "❌ <span style='color:red;'>หลุด</span>"
            
            top_nums = " • ".join(str(x) for x in rec["Hot_Top"])
            bot_nums = " • ".join(str(x) for x in rec["Hot_Bot"])
            ...
            st.markdown(
                ...
                <div style="border: 1px solid #ddd; border-radius: 8px; padding: 12px; margin-bottom: 10px; background-color: #fafafa;">
                    ...
                    <div style="font-size: 14px; margin-bottom: 4px;">
                        <b>บน ({rec['Result_3D']}):</b> {top_nums} &nbsp;👉&nbsp; {top_mark}
                    </div>
                    <div style="font-size: 14px;">
                        <b>ล่าง ({rec['Result_2D']}):</b> {bot_nums} &nbsp;👉&nbsp; {bot_mark}
                    </div>
                </div>
                """, unsafe_allow_html=True
            )
